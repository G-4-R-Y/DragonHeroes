...
It reads a `--tournament` sweep correctly too. That matters because every method
in a bracket runs as its own subprocess writing to the SAME per-key feed, so the
generation counter restarts when PPO follows ES: the window therefore sums the
sweep as per-key **fractions**, not raw generations, and a key whose ES half is
done and gated reads `1/2 · ppo 2/2`, not `100/100 PASS`. A method that emits no
feed of its own counts as half — the same rule the arena console's sweep tracker
uses, so the two never disagree.

#### The asset forge window (`tools/genforge_watch.py`)

The same window for the CONTENT pipeline, with one honest difference: a training
run emits a JSONL feed to tail, and GenForge has none — a build is a short
command that either produces an immutable bundle or does not. So this one
watches **state on disk** (releases, bundles, blockers, provenance findings,
approvals) and repaints when it changes.

```bash
tools/genforge_watch.py                     # every pack, repainting
tools/genforge_watch.py fen_bells           # one pack, with its findings
tools/genforge_watch.py --once              # one frame, for a log or a gate
tools/genforge_watch.py --interval 5        # seconds between repaints
```

It imports `tools/genforge.py`'s own `list_data`/`audit_data`, so the rules live
in exactly one place and this window can never call an asset fine when
`tools/genforge.py check` fails it. It is read-only: it never builds, approves
or rejects anything.
...
A verdict is a number; sometimes you need to see the fight. The arena console's
**VERSUS** tab has a `WATCH` next to `RUN BEST-OF-N` that spectates the exact
pairing the run would play — **both** nets attached, for the whole set
(`best_of x episodes/round`) — and `WATCH THIS` under HISTORY replays a verdict
that already ran. A bracket row is replayable too: it records `out`, the path of
the versus verdict that decided it, so watching a tournament showdown opens the
match set that actually settled the bracket.

This used to be impossible. The console's old `WATCH` hardcoded one shape — the
selected key's newest net against an opponent **build**, one episode, passing
`--policy-a` only — so a net-vs-net showdown could never be watched, which is
what Ricardo hit on 2026-09-14. The arena already accepted `--policy-b` and
`--episodes`; the console simply never sent them.
...
| dh-env → arena replay | `bash tools/trace_replay_test.sh` | `TRACE REPLAY OK` — the ranged reference replays at `drift_mean_a=0.46 drift_mean_b=0.87`, `break_tick=-1` both sides, gap `28.8 → 28.7`; the contact case prints as INFO (known divergence, R55) |
