...
| 2026-09-21 | **R55-b residual, localized** (cinder_drake v6.0 vs scripted, 32 eps) | — | totals now agree and only the CLOCK does not: damage taken dh **0.813** bars vs arena **0.892**, dealt dh **0.962** vs **0.897**, per channel within ~10% — but the episodes run dh **19.9 s** vs arena **11.3 s**. It is not a tail: the whole distribution is shifted (dh median **22.1 s**, min 8.2; arena median **9.8 s**, max 20.8, zero episodes at the cap). Restricted to the first 10 s the RATES already agree (taken dh 94.5 vs ar 106.3, dealt 101.6 vs 104.3 raw hp). So dh lands **96%** of its eventual damage inside 10 s and then spends another ten seconds on the last 4%, where the arena lands 98.8% and ends | the residual is the ENDGAME, not the exchange: below 0.25 hp `scripted_policy.gd` re-arms `_retreat_t = 2.5` forever and both bodies run at the same speed, so the chase only converges when something breaks the symmetry. The arena has `creature.gd::_separate(delta)` body separation and the sim has none — the first thing to port next. **SUPERSEDED TWICE (2026-09-22).** (a) R59 killed the stated cause: separation landed in all four runtimes and the arena never ran `_separate` either (arena bodies are `bot_drive`, so `_state` parks at `"idle"` and `_chase`, where the call lived, never executed). (b) The real cause is the row below — `creature.gd::_move` refused the whole step at the ring, so the ARENA's loser froze against the fence and died early; the sim's `clamp_disc` slid it along. The reading here is therefore right about WHERE (the endgame) and inverted about WHICH runtime was wrong: dh-env was not slow, the arena was truncated |
| 2026-09-22 | **R55-c — the endgame phase split** (`arena.gd` publishes `low_t_s`; `env_parity` derives `exchange_s`/`chase_s` in both runtimes; cinder_drake v6.0 vs scripted, 32 eps, seed 4242) | the instrument only; no behaviour change | 1v1 mirrors make the roadmap's literal split (*first death vs after first death*) degenerate — the first death IS the episode end. The measurable split is the one `scripted_policy.gd` defines: **EXCHANGE** spawn → first side below `0.25` hp, **CHASE** that moment → end. Reading: exchange dh **11.79 s** vs arena **10.87 s** (1.08×), chase dh **8.73 s** vs arena **3.58 s** (**2.44×**) — **89% of the whole clock gap is the chase**, and the exchange agrees | the question stopped being "what does the sim mis-model about combat" and became "what happens after one body turns to run". `exchange_s`/`chase_s` are deliberately NOT in `RATIO_TERMS` and do not move `agree`: they ATTRIBUTE a divergence the gate already caught |
| 2026-09-22 | **R55-c — the cause: a movement fence that refused the whole step** (`creature.gd::_move`) | one GDScript function; dh-env untouched | `_move` was all-or-nothing: `if world.is_walkable(target)` or no movement at all, so a body at the ring froze flat and could not slide ALONG the wall it touched. `Arena::clamp_disc` projects radially and KEEPS the tangential component, so the sim's retreating loser slid and lived while the arena's stood still and died. Fixed in two stages — (1) the axis-separated fallback `player.gd::_move` has always had (`hunt3d.gd:307` calls its copy "wall slide, 2D parity"), which left `dps_taken` at 1.27× against a 1.25× tolerance because two discrete axis chunks lose distance a radial projection keeps; (2) route through `ArenaWorld::clamp_inside(target, body_radius)` — `clamp_disc` exactly — when the world offers the method. cinder_drake 32 eps: win-gap 0.219→**0.000**, seconds 1.51×→**1.00×**, dps_dealt 1.43×→**1.06×**, dps_taken 1.60×→**1.01×**, chase 2.44×→**1.10×**, **PARITY OK** | also a shipping gameplay bug, not only a parity one: a fleeing creature pinned itself in a corner and died to a wall instead of to the player. Creatures were the only bodies without a wall slide |
| 2026-09-22 | **R55-c — the 64-episode matrix, run TWICE** (7 creature builds × scripted, 64 eps, seed 4242; HEAD's `_move` vs the fixed one, nothing else changed) | the one GDScript function | every dh-env column is bit-identical across the two runs — only the arena moved, as a GDScript change must. **`agree` 0/7 → 3/7**, worst ratio across the matrix **2.08× → 1.81×**, mean \|chase−1\| **1.098 → 0.415**, mean \|seconds−1\| **0.361 → 0.138**. Arena chase seconds HEAD→fixed against a fixed dh-env: cinder_drake 3.87→8.54 (dh 8.96), grave_shade 7.41→18.87 (dh 19.60), fen_boar_alpha 13.48→20.65 (dh 18.89), mire_serpent 2.65→6.29 (dh 8.58), gloamfen_stalker 3.45→8.93 (dh 6.10), gloam_wisp 8.50→11.70 (dh 4.64), bog_golem 6.39→5.90 (dh 4.29) | the fence was a ONE-DIRECTIONAL bias — every arena chase was truncated. What is left has MIXED sign, the healthier residual. Four builds converged; the two **kiting/ranged** bodies (`gloam_wisp` 1.83×→2.52×, `gloamfen_stalker` 1.77×→1.46×) overshot and now run LONGER in the arena — the sim ends their endgame too early, opposite sign, a separate cause. bog_golem's `worst` 0.234→0.422 is `win_rate` in a greedy MIRROR: near-deterministic, flips en masse; its rate ratios improved 1.14×→1.08× |
