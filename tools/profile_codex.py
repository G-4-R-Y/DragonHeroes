...
def main():
    ...
    command = [str(args.package.resolve()/'dragon-heroes-codex.x86_64')] if args.package else ['godot','--path',str(ROOT/'game')]
    command += ['--rendering-method','gl_compatibility','--','--codex-profile',f'--profile-seconds={args.seconds}']
    result_path = Path(env['XDG_DATA_HOME'])/'Dragon Heroes Codex/client-profile.json'
